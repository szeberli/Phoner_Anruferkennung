...
If Not StringInStr($CMDLINERAW, "/watcher") _
	...
	ConsoleWrite('    Environment(Language:' & @OSLang & "  Keyboard:" & @KBLayout & "  OS:" & @OSVersion & "/" & @OSServicePack & "  CPU:" & & " OS:" & @OSArch)
	ConsoleWrite(")" & @CRLF)
EndIf
...
Global $INP_Res_Language = "", $INP_RES_requestedExecutionLevel = "", $INP_RES_Compatibility = "", $INP_Fileversion = "", $INP_Fileversion_New = "", $INP_Fileversion_AutoIncrement = "", $INP_LegalCopyright = ""
...
If @OSArch <> "x86" Then
	$RegistryLM = "HKLM\Software\Wow6432Node\AutoIt v3\Autoit"
EndIf
...
If $ShowGUI And $Option = "Compile" And $InputFileIsUTF16 <> 9 Then GUI_Show()
;
...
If $Option = "Compile" Then
	...
	ElseIf $INP_AutoitDir <> "" And FileExists($INP_AutoitDir & "\aut2exe\aut2exe.exe") Then
		If @OSArch <> "x86" And FileExists($INP_AutoitDir & "\aut2exe\aut2exe_x64.exe") And StringInStr(RegRead("HKCR\AutoIt3Script\Shell\Compile\Command", ""), "aut2exe_x64.exe") Then
...
Else ; $Option = "Run"
	...
	ElseIf $INP_AutoitDir <> "" And FileExists($INP_AutoitDir & "\autoit3.exe") Then
		If @OSArch <> "x86" And $INP_UseX64 = "y" And FileExists($INP_AutoitDir & "\autoit3_x64.exe") Then
...
If Not $InputFileIsUTF32 And Not ($Option = "AU3Check") And $INP_Run_Tidy = "y" Then
	Global $TidypgmVer = ""
...
Func Retrieve_PreProcessor_Info()
	...
	Local $i_Rec_Param, $i_Rec_Value, $Temp_Val, $Fh, $UTFtype
	FileClose($hTest_UTF)
...
;~ EF BB BF UTF-8
	Select
		...
		Case BinaryMid($Test_UTF, 1, 3) = '0xFFFE0000' ; UTF-32 LE
			...
			$SrceUnicodeFlag = 0
			$InputFileIsUTF32 = 1
		Case BinaryMid($Test_UTF, 1, 3) = '0xFEFF' ; UTF-16 BE
			$UTFtype = '16BE'
			$SrceUnicodeFlag = 64
			$InputFileIsUTF16 = 1
	...
	If $InputFileIsUTF16 Then
		ConsoleWrite("! ***************************************************************************************************" & @CRLF)
		ConsoleWrite("! * Input file is UTF" & $UTFtype & " encoded, Au3Check/Obfuscator do not support UNICODE and will be skipped.*" & @CRLF)
		ConsoleWrite("! ***************************************************************************************************" & @CRLF)
	ElseIf $InputFileIsUTF32 Then
		ConsoleWrite("! ***************************************************************************************************" & @CRLF)
		ConsoleWrite("! * Input file is UTF" & $UTFtype & " encoded, Au3Check/Tidy/Obfuscator do no support UNICODE and will be skipped.*" & @CRLF)
		ConsoleWrite("! ***************************************************************************************************" & @CRLF)
	Else
		...
		If $aFile_len > StringLen($aFile_tot) Then
			...
			$SrceUnicodeFlag = 128
			ConsoleWrite("! ***************************************************************************************************************" & @CRLF)
			ConsoleWrite("! * Input file is UTF8 without BOM encoded, Au3Check/Obfuscator do not support UNICODE and will be skipped.      *" & @CRLF)
...
;~ 	GUICtrlSetTip($H_AUTOIT3_Ansi, "Check to compile with the Ansi version when Win9x/Me support is needed.")
	...
	Local $H_Res_requestedExecutionLevel_n = GUICtrlCreateRadio("None", 105, 186, Default, Default, $WS_GROUP)
	...
	Local $H_Res_requestedExecutionLevel_r = GUICtrlCreateRadio("requireAdministrator", 340, 186)
	...
	GUICtrlSetTip($H_Res_requestedExecutionLevel_n, "Specify the RequestedExecutionLevel to be set in the Program Manifest.")
	...
	$temp = "Possible Parameters: " & @CRLF & _
			...
			"  " & @CRLF & _
			"To strip the source, which is included in the ouput EXE from all Comments, Whitespace and All un-used Func's (also included UDF's), you just specify :   " & @CRLF & _
			"/striponly"
